...
def test_10s_cannot_pass_30min_gate():
    ...
    assert synthetic_endurance["verdict"] != "PASS", f"10-second run must not pass 30-min gate! Got: {synthetic_endurance['verdict']}"


def test_session_change_cancels_pending_injection():
    ...
    assert req.status == InjectionStatus.REQUESTED

    # Session 2002 connects, tearing down session 1001
    injector.cancel_pending(current_session=2002, reason="Session changed")
    assert req.status == InjectionStatus.CANCELLED
    assert req.completed_event.is_set()
    assert "Session changed" in str(req.error)


def test_session_mismatch_in_check_pause_cancels_request():
    ...
    assert ok is True
    assert pause_duration == 0.0
    assert req.status == InjectionStatus.CANCELLED
    assert "Session mismatch" in str(req.error)


def test_uncompleted_injection_cannot_claim_recovery_pass():
...
def test_downstream_rejects_dead_display():
    ...
    assert passes == 0, f"Delivered downstream verdict must reject synthetic dead display! Passed: {passes}"
