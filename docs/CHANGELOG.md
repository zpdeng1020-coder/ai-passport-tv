...
- The orphan watch was rewritten to work on Windows. Watching `os.getppid()` detects an orphan on POSIX, where a child is reparented, but on Windows the parent id never changes and the check never fires -- so the build passed on macOS and Linux and failed on Windows, which is where it was reported. The launcher now gives each child a stdin pipe it never writes to and closes on the way out; when the launcher ends for any reason at all, including being killed outright, the child's read returns end-of-file and it leaves. It is a handle rather than a value, so it cannot go stale, and nothing in it is platform-specific. Verified by `kill -9` on the launcher: before, two children survived and held both ports; after, nothing is left.
