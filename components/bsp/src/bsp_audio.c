...
esp_err_t bsp_audio_write_timeout(const void *pcm, size_t bytes, size_t *written, uint32_t timeout_ms) {
    ...
    *written = 0;
    if (!s_opened || !s_tx || s_bits != 16 || s_ch != 1) return ESP_ERR_INVALID_STATE;
    if (!pcm || !bytes || bytes % 2) return ESP_ERR_INVALID_ARG;
    return i2s_channel_write(s_tx, pcm, bytes, written, timeout_ms);
}
...
esp_err_t bsp_audio_stream_close(void) {
    ...
    s_opened = false;
    // Keep the established close/open contract: open disables running channels.
    if (s_tx) i2s_channel_enable(s_tx);
    if (s_rx) i2s_channel_enable(s_rx);
    return ESP_OK;
}
...
void bsp_audio_set_mute(bool mute) {
    if (s_dev) esp_codec_dev_set_out_mute(s_dev, mute);
}
