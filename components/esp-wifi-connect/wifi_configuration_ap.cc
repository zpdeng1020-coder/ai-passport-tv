...
bool WifiConfigurationAp::SetOtaUrl(const std::string &url)
{
    ota_url_ = url;
    nvs_handle_t nvs;
    esp_err_t err = nvs_open("wifi", NVS_READWRITE, &nvs);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to open NVS to save the server address: %d", err);
        return false;
    }
    ...
    nvs_close(nvs);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to save the server address: %d", err);
        return false;
    }
    ESP_LOGI(TAG, "Server address saved (%u characters)", (unsigned)ota_url_.size());
    return true;
}
...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t form_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // The streaming server's address, submitted on the same form.
            // Bounded like every other value taken from a request: the field is
            // free text and this buffer is fixed.
            //
            // An empty value leaves the stored address alone. This form is
            // submitted every time a network is configured, and someone joining
            // a second network has no reason to retype the server; overwriting
            // it with "" would silently break a device that was working. This
            // matches what the page tells the reader.
            cJSON *server_item = cJSON_GetObjectItemCaseSensitive(json, "ota_url");
            if (cJSON_IsString(server_item) && server_item->valuestring != NULL) {
                std::string server_str = server_item->valuestring;
                const bool blank =
                    server_str.find_first_not_of(" \t\r\n") == std::string::npos;
                if (server_str.size() < 128 && !blank) {
                    this_->SetOtaUrl(server_str);
                }
            }
