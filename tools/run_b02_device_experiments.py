...
def run_experiment_downstream_pauses(runner: DeviceExperimentRunner) -> List[Dict[str, Any]]:
    ...
    for pause_s in [0.5, 1.0, 3.0]:
        for rep in range(1, 4):
            ...
            # 1. Baseline verification: require active streaming
            cur_session = getattr(runner.server, "session_id", 0)
            v_start = getattr(runner.server, "video_sent", 0)
            a_start = getattr(runner.server, "audio_sent", 0)

            # Wait and check baseline advancement
            time.sleep(1.0)
            cur_session = getattr(runner.server, "session_id", 0)
            v_pre = getattr(runner.server, "video_sent", 0)
            a_pre = getattr(runner.server, "audio_sent", 0)

            # Rigorous check: must have active video advancing prior to injection
            if cur_session == 0 or v_pre == 0 or v_pre <= v_start:
                runner.log_server(f"FAILED baseline for {label}: cur_session={cur_session} v_pre={v_pre} v_start={v_start}")
                rec = {
                    "label": label,
                    ...
                    "video_sent_after": v_pre,
                    "video_delta": 0,
                    "audio_delta": 0,
                    "underflow_observed": False,
                    "verdict": "FAIL",
                    "reason": "Pre-injection baseline check failed: no active video stream"
                }
                results.append(rec)
                continue
            ...
            runner.log_server(f"Observing post-pause recovery for {observe_duration}s (from {t_supply_resumed:.3f})...")
            
            t_obs_start = time.monotonic()
            while time.monotonic() - t_obs_start < observe_duration:
            ...
            actual_observe_s = round(t_obs_end - t_obs_start, 2)
            ...
            # Check device window metrics
            device_fps_met = True
            if fresh_windows:
                total_w_frames = sum(w["frames"] for w in fresh_windows)
                total_w_ms = sum(w["interval_ms"] for w in fresh_windows)
                dev_fps = round(total_w_frames * 1000.0 / total_w_ms, 2) if total_w_ms > 0 else 0.0
                if dev_fps < 10.0:
            ...
            # - video and audio continued advancing after resume (v_delta > 0, a_delta > 0)
            # - post-recovery server average fps >= 10.5 fps
            # - device fps met if windows available
            is_pass = (
                injection_completed and
                same_session and
                v_delta >= int(observe_duration * 9.0) and  # At least 9 fps overall
            ...
            runner.log_server(
                ...
                f"v_delta={v_delta} a_delta={a_delta} fps={server_rendered_fps} underflow={underflow_observed}"
            )
            time.sleep(2.0)
...
def run_experiment_upstream_pauses(runner: DeviceExperimentRunner) -> List[Dict[str, Any]]:
    print("\n" + "="*70)
    print("EXPERIMENT 3: Upstream Transcode Pauses (0.5s, 1.0s, 3.0s x 3)")
    print("="*70)
    results = []

    for pause_s in [0.5, 1.0, 3.0]:
        for rep in range(1, 4):
            label = f"upstream_{pause_s}s_rep{rep}"
            runner.log_server(f"--- Preparing {label} ---")
            ...
            actual_observe_s = round(time.monotonic() - t_obs_start, 2)
            v_post = getattr(runner.server, "video_sent", 0)
            a_post = getattr(runner.server, "audio_sent", 0)
            v_delta = v_post - v_at_resume
            a_delta = a_post - a_at_resume
            ...
            rec = {
                ...
                "session_id": cur_session,
                "experiment_id": exp_id,
                "qa_before": inject_res["qa_before"],
                "qa_after": inject_res["qa_after"],
                "absorbed_by_host": inject_res["absorbed_by_host"],
                "video_delta": v_delta,
                "audio_delta": a_delta,
            ...
            runner.log_server(
                ...
                f"absorbed={rec['absorbed_by_host']} v_delta={v_delta} fps={server_fps}"
            )
            time.sleep(2.0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--full-matrix", action="store_true", help="Run full 0.5s, 1.0s, 3.0s matrix")
    ...
    try:
        ...
        if args.single_control:
            ...
            all_passed = (
                reconnect_res["verdict"] == "PASS" and
                all(r["verdict"] == "PASS" for r in downstream_res) and
                all(r["verdict"] == "PASS" for r in upstream_res)
            )
            sys.exit(0 if all_passed else 1)

        else:
            # Default: run reconnect and single control
            reconnect_res = run_experiment_reconnect(runner)
            ctrl_res = run_single_control_experiment(runner, pause_s=args.pause, observe_s=args.observe)
            summary = {
                "reconnect_experiment": reconnect_res,
