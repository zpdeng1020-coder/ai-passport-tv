...
# Third-party code kept verbatim from upstream. The documentation rules below
# are for documents this project writes, and both of them would have to be
# satisfied by editing files that are deliberately byte-identical to upstream:
# a vendored README links to its own sources, and adding a translation of it
# would make the copy differ from the original it is verified against. The
# security checks -- secrets and conflict markers -- still run on these paths,
# because a vendored file can carry either.
VENDORED_PREFIXES = (
...
def is_vendored(path: Path) -> bool:
    ...
    return relative.startswith(VENDORED_PREFIXES)
...
def check_markdown_links(files: list[Path], errors: list[str]) -> None:
    for path in files:
        if path.suffix.lower() != ".md" or is_vendored(path):
            continue
...
def check_document_languages(files: list[Path], errors: list[str]) -> None:
    ...
    markdown = {
        path.resolve()
        for path in files
        if path.suffix.lower() == ".md" and not is_vendored(path)
    }
