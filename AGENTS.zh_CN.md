...
- 必须保持受保护的 Flash 布局：3 MB 应用上限与 `cardid@0x356000`
  均为模板强制契约。
- 构建、测试与修复一律使用**公开版**（`-D AV_PUBLIC_BUILD=ON`），不得使用本地版。
  公开版是使用者拿到的版本，也是必须在全新设备上跑通的配置；编译期带凭据的本地版
  会掩盖只有公开版才会暴露的问题——未配置的服务器地址、未保存的网络、配网流程本身。
  本地版仅用于复现某一条具体反馈，绝不能作为默认测试路径。
...
commit 是本地操作；push、release、上传制品、推送镜像都不是，每一项都需要用户针对该次操作明确同意。**允许提交不等于允许发布。**

## 未经测试的部分必须如实说明

没跑过的部分要和跑过的部分一样如实交代。开发机是 macOS：`run.bat` 以及
`tools/launch.py` 里 Windows 分支的写法来自命令的公开文档，从未在 Windows 上运行过。
这一点要写在读者看得到的地方，不能因为文件存在就让人以为它验证过。
